...
graph TB
    subgraph edgeA["Edge Region A"]
        dpA["<b>Edge proxy</b><br/>+ Cache"]
    end

    subgraph edgeB["Edge Region B"]
        dpB["<b>Edge proxy</b><br/>+ Cache"]
    end

    subgraph core["Core Data Center"]
        admin["<b>Admin (Talos)</b>"]
    end

    dpA -- verify upstream --> admin
    dpB -- verify upstream --> admin
    admin --> db[("Database")]
```
...
server. It intercepts `POST` requests to `/v2alpha1/admin/apiKeys:verify`, caches positive (active)
...
- The request is a `POST` to `/v2alpha1/admin/apiKeys:verify`
- The upstream returned HTTP `200 OK`
- The response body contains `"is_valid": true`
