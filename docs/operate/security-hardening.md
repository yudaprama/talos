...
- **Restrict admin endpoints** to internal networks only. Admin endpoints have no built-in
  authentication and must sit behind a trusted proxy that authenticates and authorizes every
  request. See [Admin protection](./security/admin-protection.md) for the supported deployment
  patterns.
- **Use TLS** for all connections. Configure database `sslmode=verify-full` and serve HTTPS (or
  terminate TLS at the load balancer).
- **Run admin and self-service in separate processes** (`talos serve admin` and
  `talos serve public`) when you need independent network or security boundaries.
