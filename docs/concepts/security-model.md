...
## Admin and self-service separation

The admin surface (management and verification) and the self-service surface (proof-of-possession
self-revocation) can be deployed as separate processes. The admin surface must be restricted to
internal networks behind a trusted proxy — see
[Admin protection](../operate/security/admin-protection.md). The self-service surface is designed to
