...
Send a request to the admin endpoint to create a new key:
...
Send the secret to the admin verify endpoint to check whether a credential is valid:
...
The response includes `is_valid` (boolean — `true` when verification succeeded), `status` (key
lifecycle state), `key_id`, `actor_id`, `scopes`, `metadata`, and `expire_time` when valid. When
`is_valid` is `false`, `error_code` and `error_message` indicate the reason. When rate limit
enforcement is enabled (Commercial), the response also includes `rate_limit_remaining` and
`rate_limit_reset_time` for keys with a rate limit policy. For the complete field reference, see the
